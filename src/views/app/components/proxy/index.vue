      ...
      <template #bodyCell="{ column, record,text }">
        <template v-if="column.key === 'note'">
          <Popover trigger="hover" placement="left">
            <template #content>
             <template v-if="noteHistoryArr.length!=0"> <p v-for="(item,index) in noteHistoryArr" :key="index"><span>时间:{{formatDate(item.created)}}</span> &emsp;<span>是否为VPN端口:{{ record.is_vpn_proxy == 1? '是' : '否'
                  }}</span> &emsp;{{ item.note }}</p></template>
             <template v-else>
                <p>暂无数据</p>
              </template>
        ...
        <template v-if="column.key === 'action'">
          ...
          <Popconfirm @confirm="confirmPrivacy(record)" @cancel="cancel" placement="left">
            ...
            <a-button color="success" style="margin-left: 10px;">设为VPN代理</a-button>
          </Popconfirm>
        </template>
      </template>
    </BasicTable>
...
import { stop,setVpn,noteHistory } from '/@/api/app';
...
const confirm = async (record) => {
  await stop({ table: 'app_proxy', id: record.id, note: note.value });
  emit('success');
  note.value = ''
};
const confirmPrivacy = async (record) => {
  await setVpn({  id: record.id, note: note.value,table: 'app_window' });
  emit('success');
  note.value = ''
};
const handleAdd = () => {
